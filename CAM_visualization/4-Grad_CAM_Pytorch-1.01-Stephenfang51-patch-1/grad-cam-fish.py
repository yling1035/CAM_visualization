...
class GradCam():
    ...
    def __call__(self, input):
        ...
        cam = cv2.resize(cam, (260, 260)) #将14*14的featuremap 放大回224*224
        cam = cam - np.min(cam)
        cam = cam  / np.max(cam)
        return cam
...
#grad_cam = GradCam(model = model, target_layer_names = ["22"])
target_layer_names =get_last_conv_name(model)
grad_cam = GradCam(model = model, target_layer_names=target_layer_names)
...
img = np.float32(cv2.resize(img, (260,260))) / 255 #为了丢到vgg16要求的224*224 先进行缩放并且归一化
